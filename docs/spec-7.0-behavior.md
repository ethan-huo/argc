...
```
Scenario: reject a top-level command/group key that collides with an argc builtin
  Given a schema with a key "@schema"
  When cli() is constructed
  Then it throws  "Invalid command key: @schema"  (not at runtime — at construction)

Scenario: accept a non-builtin command/group key that starts with `@`
...
Scenario: @-prefix dispatch is position-aware
  `cli @run ...`    → first token @run = built-in
  `cli @foo ...`    → app command when @foo is present in the schema
